...
process_execute (const char *file_name) 
{
	...
	fn_copy2 = strtok_r(fn_copy2, " ", &tmp);
...
start_process (void *fname)
{
	...
	if(success)
	{
		//uint32_t endofstack = (uint32_t)if_.esp;
		int len,i, argc = 1;
		...
		if_.esp -= (unsigned)if_.esp % 4; //word-align
		if_.esp -= 4;
		*(unsigned*)if_.esp = 0;
		...
		*(unsigned*)if_.esp = 0;
		...
		//hex_dump(0, if_.esp, endofstack-(unsigned)if_.esp, true);
